...
public class DownloaderIntentService extends IntentService {
	...
	private void download() {
		...
		if (jsonStr != null) {
			...
			try {
				...
				getContentResolver().delete(UserProvider.CONTENT_URI, where,
						args);
				insertAll(users);
	...
	private void sendNotification(boolean result) {
		...
		if (!result) {
			...
			contentText = context.getText(R.string.notification_info_success)
					.toString();
		}
		...
		Intent notificationIntent = new Intent(context, UserListActivity.class);
	...
	private boolean insertAll(JSONArray users) {
		...
		try {
			while (i < users.length()) {
				...
				getContentResolver().insert(UserProvider.CONTENT_URI, userData);
				i++;
			}
			result = true;
		} catch (JSONException e) {
